...
class WebServer:
    ...
    def start_http_server(self):
        while True:
            ...
            socket_client, client_addr = self.server_socket.accept()

            # 这个时候子进程会把主进程里面的变量复制一遍 只在一方修改的时候进行分离
            # 由于子进程复制了这个socket_client 所以需要在主进程关闭socket
            this_process = Process(target=self.dispatch_request, args=(socket_client,))
            this_process.start()
            socket_client.close()

    def dispatch_request(self, socket_client):
        """
        处理socket链接
        :param socket_client:
        :return:
        ...
        if not request_header_lines:
            ...
            file_name = self.get_the_file_name(request_header_lines[0]) or "index.html"
        ...
        try:
            ...
            response_headers += "\r\n"  # 空一行与body隔开
            # 设置内容body
            response_body = self.find_static_file_data(file_name)
        except:
...
def main():
    ...
    server = WSGIServer(("0.0.0.0", 8081), app.application)
